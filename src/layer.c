...
static void layer_init_linear(struct l_layer *layer) {
    layer->state = 0;
    layer->weights = malloc(sizeof(float) * layer->out_size * (/*biases*/ 1 + /*weights*/ layer->in_size));
}
...
static void layer_init_quadratic(struct l_layer *layer) {
    layer->state = 0;
    layer->weights = malloc(sizeof(float) * layer->out_size * (/*biases*/ 1 + /*weights*/ layer->in_size * 2));
}
...
void l_layer_process(struct l_layer *layer, float *inputs, float *outputs) {
    layer->type->process(layer, inputs, outputs);
}
